 ...
 * Recibe el email y la nueva contraseña, la actualiza (sin verificación de email por ahora).
 */
export const forgotPassword = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const result = await db.execute({
      sql: `SELECT id FROM users WHERE email = ? AND activo = 1`,
      args: [email],
    })

    if (result.rows.length === 0) {
      // Por seguridad, no revela si el usuario existe o no
      res.status(200).json({ success: true, message: 'Si el correo existe, la contraseña fue actualizada.' })
      return
    }
