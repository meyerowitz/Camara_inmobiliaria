...
const cursos = [
  { id: 1, titulo: 'Diplomado en Derecho Inmobiliario', instructor: 'Abog. Luis Martínez', precio: '45.00', rating: 4.9, img: 'https://images.unsplash.com/photo-1589829545856-d10d557cf95f?auto=format&fit=crop&q=80&w=800', nivel: 'Avanzado' },
  { id: 2, titulo: 'Marketing Digital para Real Estate', instructor: 'Lic. Elena Pérez', precio: '29.99', rating: 4.8, img: 'https://images.unsplash.com/photo-1460925895917-afdab827c52f?auto=format&fit=crop&q=80&w=800', nivel: 'Principiante' },
  { id: 3, titulo: 'Tasación de Inmuebles Urbanos', instructor: 'Ing. Carlos Ruiz', precio: '35.00', rating: 4.7, img: 'https://www.cosasdevalor.com/images/categorias_valuatorias/inmuebles-urbanos.jpg', nivel: 'Intermedio' },
  { id: 4, titulo: 'Neuroventas para Corredores', instructor: 'Coach Mario Vargas', precio: '19.99', rating: 5.0, img: 'https://images.unsplash.com/photo-1552664730-d307ca884978?auto=format&fit=crop&q=80&w=800', nivel: 'Intermedio' }
]
  ...
  return (
    <section id='cursos' className={`relative min-h-screen transition-all duration-700 px-6 lg:px-20 py-24 scroll-mt-24 ${darkMode ? 'bg-[#011a14] text-white' : 'bg-[#f8fafc] text-slate-900'}`}>
      ...
      <div className='grid sm:grid-cols-2 lg:grid-cols-4 gap-8 relative z-10'>
        {cursos.map((curso) => (
          <div key={curso.id} className={`group rounded-[2.5rem] overflow-hidden border-2 transition-all duration-500 hover:-translate-y-3 ${darkMode ? 'bg-[#022c22]/50 backdrop-blur-md border-white shadow-[0_20px_50px_rgba(0,0,0,0.3)]' : 'bg-white border-emerald-50 shadow-[0_15px_35px_rgba(16,185,129,0.05)] border-emerald-500'}`}>
            <div className='relative h-56 overflow-hidden'>
              <img src={curso.img} alt={curso.titulo} className='w-full h-full object-cover group-hover:scale-110 transition duration-1000' />
              <div className='absolute inset-0 bg-gradient-to-t from-black/60 to-transparent' />
              <div className='absolute top-5 left-5 bg-emerald-500 text-[#022c22] px-4 py-1.5 rounded-xl text-[10px] font-black uppercase shadow-lg'>{curso.nivel}</div>
            </div>
            <div className='p-8 space-y-4'>
              ...
              <h3 className={`font-black text-xl leading-[1.2] transition-colors min-h-[3rem] ${darkMode ? 'text-white group-hover:text-emerald-400' : 'text-[#022c22] group-hover:text-emerald-600'}`}>{curso.titulo}</h3>
              <p className={`text-xs font-bold ${darkMode ? 'text-emerald-200/40' : 'text-slate-400'}`}>CON: <span className={darkMode ? 'text-emerald-400' : 'text-emerald-600'}>{curso.instructor.toUpperCase()}</span></p>
              ...
              <div className='flex items-center justify-between pt-2'>
                ...
                <button className={`p-4 rounded-2xl transition-all active:scale-90 ${darkMode ? 'bg-emerald-500/10 text-emerald-500 hover:bg-emerald-500 hover:text-[#011a14]' : 'bg-emerald-500 text-white hover:bg-[#022c22] shadow-lg shadow-emerald-200'}`}>
