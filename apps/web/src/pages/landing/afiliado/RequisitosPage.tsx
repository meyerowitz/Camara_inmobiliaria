...
const AFFILIATION_STEPS = [
  {
    ...
    details: (
      <>
        Datos del aspirante (Nombre, Cédula, RIF, Correo), experiencia como asesor y tipo de persona (Natural/Jurídica).
      </>
    ),
    icon: ClipboardList,
    ...
    details: (
      <>
        ...
        <ul className="text-left list-disc pl-3 mt-1.5 space-y-1">
          <li><b>Ingreso Directo:</b> Si tu perfil cumple todos los requisitos.</li>
          <li><b>Nivelación:</b> Cursarás solo los módulos del CIBIR que necesites reforzar.</li>
                    ...
                    >
                      <div className="py-10 px-5 h-full flex flex-col items-center text-center space-y-4 pr-10">
                        ...
                        <div className="flex flex-col items-center flex-1 w-full justify-start overflow-hidden">
                          <span className="text-[9px] font-black text-white/70 uppercase tracking-widest bg-black/10 px-2.5 py-0.5 rounded-full mb-2 shrink-0">
                            Paso 0{idx + 1}
                          </span>
                          <h3 className="font-black text-white text-xs uppercase tracking-wider shrink-0 mb-1">{step.title}</h3>
                          
                          {/* Short Description - Always visible */}
                          <p className="text-[10px] text-white/90 font-medium leading-tight mb-2 shrink-0">
                            {step.description}
                          </p>
                          ...
                          <div className="grid grid-rows-[0fr] group-hover:grid-rows-[1fr] transition-all duration-500 ease-in-out w-full">
                            <div className="overflow-hidden">
                              <div className="text-[10px] text-white/80 leading-relaxed font-normal pt-3 border-t border-white/20 mt-1 opacity-0 group-hover:opacity-100 transition-opacity duration-500 delay-100">
                                {step.details}
                              </div>
                            </div>
                          </div>
                  ...
                  >
                    ...
                    <div className="flex items-center gap-4 relative z-10">
                      ...
                      <div className="min-w-0">
                        <p className="text-[8px] font-black uppercase tracking-widest opacity-30 mb-0.5">Paso 0{idx + 1}</p>
                        <h3 className="font-black text-slate-800 text-xs uppercase tracking-tight">{step.title}</h3>
                      </div>
                    </div>
                    <div className="relative z-10 border-t border-slate-50 pt-3">
                      <p className="text-[11px] font-bold text-slate-700 mb-1">{step.description}</p>
                      <div className="text-[10px] text-slate-500 font-medium leading-relaxed">{step.details}</div>
                    </div>
                  </div>
                ))}
              </div>
